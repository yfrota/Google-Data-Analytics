...
###
tercateste<-sqldf ('SELECT SedentaryMinutes, diadasemana FROM semana1 WHERE diadasemana = "2-segunda"')


tercateste<-sqldf ('SELECT sum(SedentaryMinutes) as soma1, diadasemana FROM semana1 WHERE diadasemana = "2-segunda"')
nrow(tercateste)
...
nrow(semanasono)
head(semanasono) 
n_distinct(semanasono$diadasemana)
...
semanasono %>% 
  group_by(diadasemana) %>% 
  summarise(totalsono = mean(TotalMinutesAsleep)) %>% 
  geom_boxplot(mapping=aes(x=mean(TotalMinutesAsleep), y= totalsono/60, fill = diadasemana ))+
  labs (title = "PERDA CALORICA vs DIA DA SEMANA ", subtitle = "de usuários de smart devices", 
        caption= "Data criada por Yuri Faria Frota", x = "Dia da semana", y = "Perda Calórica")

uninstall.packages ("LaTeX")
